...
# 文件读取缓存（使用lru_cache + mtime校验）
_cache_lock = threading.Lock()
_file_cache: Dict[str, Tuple[float, str, float]] = {}
...
def _read_text_safe(path: str) -> str:
    """多编码安全读取，带内存缓存（mtime校验），失败时抛出NuwaError"""
    current_time = time.time()
    normalized_path = _normalize_path(path)
    file_mtime = _get_file_mtime(normalized_path)
    
    # 检查缓存（带锁保护）
    with _cache_lock:
        if normalized_path in _file_cache:
            cache_time, cached_content, cached_mtime = _file_cache[normalized_path]
            if current_time - cache_time < _CACHE_TTL and cached_mtime == file_mtime:
                return cached_content

    last_error = None
    for enc in ("utf-8", "gbk", "gb18030"):
        try:
            with open(normalized_path, encoding=enc, errors="replace") as f:
                content = f.read()
                # 更新缓存（带锁保护）
                with _cache_lock:
                    _file_cache[normalized_path] = (current_time, content, file_mtime)
                return content
...
def process_batch(input_files: List[str], output_dir: Optional[str] = None, max_workers: int = 4) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for idx, filepath in enumerate(input_files, 1):
            print(f"进度: {idx}/{total} 完成", file=sys.stderr)
            result = _process_file_with_retry(filepath)
            if result["success"]:
                results.append(result)
                if output_dir:
                    os.makedirs(output_dir, exist_ok=True)
