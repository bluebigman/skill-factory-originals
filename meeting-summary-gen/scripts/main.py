# -*- coding: utf-8 -*-
"""main.py — 会议纪要 会议总结生成器 CLI 引擎（规则模板生成器）
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        prog="meeting-summary-gen",
        description="把会议记录整理成议题、决议、行动项与责任人的结构化会议总结。（离线模板引擎，不写盘预览优先）",
    )
    p.add_argument("--topic", default="", help="主题/内容（必填一：topic 或 input）")
    p.add_argument("--input", default="", help="输入文件路径（多编码容错；与 topic 二选一）")
    p.add_argument("--mode", default="standard", choices=sorted(SCENE_OPTIONS.keys()))
    ...
    p.add_argument("--version", action="version", version="meeting-summary-gen " + VERSION)
...
def _safe_main(argv=None):
    ...
    for idx in range(args.count):
        docs.append(build_doc(raw, args.mode, idx, rng, log))

    scene_label = SCENE_OPTIONS.get(args.mode, args.mode)
    if args.as_json:
        payload = json.dumps({"generator": "meeting-summary-gen", "version": VERSION,
