# -*- coding: utf-8 -*-
"""main.py — SQL查询 语句生成助手 CLI 引擎（规则模板生成器）
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        prog="sql-query-assistant",
        description="按表结构描述生成 SQL 查询语句，附带执行逻辑解释与优化建议。（离线模板引擎，不写盘预览优先）",
    )
    p.add_argument("--topic", default="", help="主题/内容（必填一：topic 或 input）")
    p.add_argument("--input", default="", help="输入文件路径（多编码容错；与 topic 二选一）")
    p.add_argument("--dialect", default="mysql", choices=sorted(SCENE_OPTIONS.keys()))
    ...
    p.add_argument("--version", action="version", version="sql-query-assistant " + VERSION)
...
def _safe_main(argv=None):
    ...
    for idx in range(args.count):
        docs.append(build_doc(raw, args.dialect, idx, rng, log))

    scene_label = SCENE_OPTIONS.get(args.dialect, args.dialect)
    if args.as_json:
        payload = json.dumps({"generator": "sql-query-assistant", "version": VERSION,
