...
# -*- coding: utf-8 -*-
"""
pdf-invoice-parser — 增值税发票字段提取（原创实现 v2.0）
...
def pull_text(path, ocr=True, password=None):
...
def parse_fields(text):
    b = Bill()
    m = _CODE_PAT.search(text);      b.code = m.group(1) if m else ""
    m = _NO_PAT.search(text);        b.number = m.group(1) if m else ""
    m = _DATE_PAT.search(text)
    if m:
        b.date = f"{m.group(1)}-{int(m.group(2)):02d}-{int(m.group(3)):02d}"
    m = _TOTAL_PAT.search(text);     b.total = m.group(1) if m else ""
    m = _TOTALCN_PAT.search(text);   b.total_cn = m.group(1) if m else ""
    m = _RATE_PAT.search(text);      b.rate = m.group(1) if m else ""
...
def validate(b):
    ...
    if None not in (amt, tax, tot):
        ...
        res["C1_sum"] = {"passed": diff <= Decimal("0.01"),
    ...
    if cn is not None and tot is not None:
        res["C2_cn"] = {"passed": abs(cn - tot) <= Decimal("0.01"),
                        "detail": f"大写 {cn} vs 小写 {tot}"}
    else:
        res["C2_cn"] = {"passed": None, "detail": "大写缺失或无法解析"}
    n = b.number
    res["C3_no"] = {"passed": bool(n) and len(n) in (8, 20),
...
# ------------------------- 主流程 -------------------------
def parse_one(path, ocr=True, password=None):
...
def write_out(rows, out_dir, fmt):
    try:
...
def selftest():
    ...
    validate(b)
    cases = [("发票代码", b.code, "033002100211"),
    ...
    for txt, want in [("壹仟壹佰叁拾元整", "1130.00"), ("贰佰零伍元陆角伍分", "205.65"),
                      ("壹万贰仟元整", "12000.00")]:
...
def main():
    ...
    if rows:
        try:
            ...
            log.error(str(e))
            return 1
    summary = {"total": len(pdfs), "ok": len(rows), "fail": len(fails),
