#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""selftest.py — talking-script-gen 自检契约（9 项断言）"""
import io
import os
import sys
import tempfile
...
def test_cli_generate():
    try:
        import run as r
        rc = r.main(["--topic", "无线降噪耳机", "--scene", "promo",
                     "--tone", "passion", "--words", "300", "--seed", "5"])
...
def test_output_shape():
    try:
        ...
        sys.stdout = buf
        try:
            r.main(["--topic", "如何坚持早起", "--scene", "short",
                    "--count", "2", "--json", "--seed", "1"])
        finally:
            sys.stdout = old
        payload = _json.loads(buf.getvalue())
        items = payload.get("items", [])
        ok = len(items) == 2 and all(len(s["paras"]) >= 3 for s in items)
        check("JSON 输出结构(count=2)", ok)
    except Exception as e:
        check("JSON 输出结构(count=2)", False, str(e))
...
def test_write_out():
    try:
        ...
        rc = r.main(["--topic", "测试口播", "--out", target])
...
def test_encoding():
    try:
        ...
        with io.open(tmp, "w", encoding="gbk") as fh:
            fh.write(u"家人们 真香 划重点")
        content = r.read_text_safe(tmp)
        check("read_text_safe gbk 容错", u"划重点" in content)
    except Exception as e:
...
def test_compliance_block():
    try:
        ...
        sys.stdout = buf
        try:
            rc = r.main(["--topic", "保证赚" + "钱的理财课"])
        finally:
            sys.stdout = old
        check("禁止词拦截(rc=3)", rc == 3, "rc=%s" % rc)
    except Exception as e:
        check("禁止词拦截(rc=3)", False, str(e))


def test_bad_args():
    try:
        import run as r
        rc = r.main(["--topic", "x", "--words", "99999"])
        check("字数越界报错(rc=2)", rc == 2, "rc=%s" % rc)
    except Exception as e:
        check("字数越界报错(rc=2)", False, str(e))
