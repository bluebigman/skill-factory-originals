        ...
        if fmt == "auto":
            ...
            if "," in stripped or "\t" in stripped:
                try:
                    ...
                    print(f"[WARN] 自动解析降级为 lines 模式: {e}", file=sys.stderr)
                    # 记录结构化日志供审计
                    log_entry = {
...
def fetch_url(url: str, timeout: int = 10, max_retries: int = 3) -> str:
    ...
    for attempt in range(max_retries):
        try:
            ...
            # SSL 证书验证失败，降级为不验证证书
            last_error = exc
            print(f"[WARN] SSL 证书验证失败，尝试不验证证书: {exc}", file=sys.stderr)
            try:
            ...
            if attempt < max_retries - 1:
                # 指数退避：2^attempt * 1s
                wait_time = 2 ** attempt
                print(f"[WARN] URL 访问失败（尝试 {attempt + 1}/{max_retries}），"
                      f"{wait_time}s 后重试: {exc}", file=sys.stderr)
...
def process_batch(input_dir: str, output_dir: str, output_format: str = "json",
    ...
    for filename in os.listdir(input_dir):
        ...
        try:
            ...
            print(f"[ERROR] 处理 {filename} 失败: {exc}", file=sys.stderr)
        except Exception as exc:
            results[filename] = f"error: E010"
            print(f"[ERROR] 处理 {filename} 发生未知错误: {exc}", file=sys.stderr)
