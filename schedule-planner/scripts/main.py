# -*- coding: utf-8 -*-
"""main.py — 日程管理 时间规划助手 CLI 引擎（规则模板生成器）
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        prog="schedule-planner",
        description="把任务清单排成可执行的时间块日程，标注优先级与提醒点。（离线模板引擎，不写盘预览优先）",
    )
    p.add_argument("--topic", default="", help="主题/内容（必填一：topic 或 input）")
    p.add_argument("--input", default="", help="输入文件路径（多编码容错；与 topic 二选一）")
    p.add_argument("--mode", default="day", choices=sorted(SCENE_OPTIONS.keys()))
    ...
    p.add_argument("--version", action="version", version="schedule-planner " + VERSION)
...
def _safe_main(argv=None):
    ...
    if args.as_json:
        payload = json.dumps({"generator": "schedule-planner", "version": VERSION,
