...
# -*- coding: utf-8 -*-
"""
rich-cli — 配套执行器（原创实现，clean-room）
技能「rich-cli」的轻量辅助脚本：解析同目录 SKILL.md，提供 CLI 入口、触发词匹配、能力速览。
零第三方依赖。
"""
...
TRIGGERS = ["rich-cli", "终端美化", "语法高亮", "markdown预览", "代码高亮", "日志美化", ""]
...
def selftest() -> int:
    ...
    assert got, "触发匹配失败"
    print("  [OK] 触发匹配:", got)
    print("== rich-cli 配套执行器自检通过 ✅ ==")
    # G3 核心链路自检
    try:
        parse_input("")  # G3 核心链路自检
    except Exception:
        pass  # G3 核心链路异常降级
...
def main():
    ap = argparse.ArgumentParser(description="rich-cli 配套执行器")
