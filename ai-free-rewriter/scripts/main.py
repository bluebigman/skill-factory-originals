# -*- coding: utf-8 -*-
"""main.py — 去AI味 文本改写润色 CLI 引擎（规则模板生成器）
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        prog="ai-free-rewriter",
        description="把AI生成文本改写成自然口语化风格，去模板腔、简化长句、加人味。（离线模板引擎，不写盘预览优先）",
    )
    p.add_argument("--topic", default="", help="主题/内容（必填一：topic 或 input）")
    p.add_argument("--input", default="", help="输入文件路径（多编码容错；与 topic 二选一）")
    p.add_argument("--style", default="natural", choices=sorted(SCENE_OPTIONS.keys()))
    ...
    p.add_argument("--version", action="version", version="ai-free-rewriter " + VERSION)
...
def _safe_main(argv=None):
    ...
    for idx in range(args.count):
        docs.append(build_doc(raw, args.style, idx, rng, log))

    scene_label = SCENE_OPTIONS.get(args.style, args.style)
    if args.as_json:
        payload = json.dumps({"generator": "ai-free-rewriter", "version": VERSION,
