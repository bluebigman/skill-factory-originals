# -*- coding: utf-8 -*-
"""main.py — 公众号文章 图文生成器 CLI 引擎（规则模板生成器）
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        prog="wechat-article-writer",
        description="按主题生成公众号长文：标题、导语、章节、金句与结尾引导。（离线模板引擎，不写盘预览优先）",
    )
    p.add_argument("--topic", default="", help="主题/内容（必填一：topic 或 input）")
    p.add_argument("--input", default="", help="输入文件路径（多编码容错；与 topic 二选一）")
    p.add_argument("--genre", default="insight", choices=sorted(SCENE_OPTIONS.keys()))
    ...
    p.add_argument("--version", action="version", version="wechat-article-writer " + VERSION)
...
def _safe_main(argv=None):
    ...
    for idx in range(args.count):
        docs.append(build_doc(raw, args.genre, idx, rng, log))

    scene_label = SCENE_OPTIONS.get(args.genre, args.genre)
    if args.as_json:
        payload = json.dumps({"generator": "wechat-article-writer", "version": VERSION,
