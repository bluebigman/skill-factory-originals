# -*- coding: utf-8 -*-
"""main.py — 活动策划方案生成器 CLI 引擎（规则模板生成器）
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        ...
        description="按目标、预算与人群生成活动主题、流程、预算与传播方案。（离线模板引擎，不写盘预览优先）",
    )
    p.add_argument("--topic", default="", help="主题/内容（必填一：topic 或 input）")
    p.add_argument("--input", default="", help="输入文件路径（多编码容错；与 topic 二选一）")
    p.add_argument("--kind", default="promo", choices=sorted(SCENE_OPTIONS.keys()))
    ...
    p.add_argument("--version", action="version", version="campaign-planner " + VERSION)
...
def _safe_main(argv=None):
    ...
    for idx in range(args.count):
        docs.append(build_doc(raw, args.kind, idx, rng, log))

    scene_label = SCENE_OPTIONS.get(args.kind, args.kind)
    if args.as_json:
        payload = json.dumps({"generator": "campaign-planner", "version": VERSION,
