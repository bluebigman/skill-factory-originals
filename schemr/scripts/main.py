...
def _read_text_safe_cached(path: str) -> str:
    """多编码安全读取（带缓存）"""
    last_error = None
    for enc in ("utf-8", "gbk", "gb18030"):
        try:
            ...
            continue
    # 如果所有编码都失败，抛出标准错误
    raise SchemrError("E002", f"文件读取失败: {path} - {last_error}")


def _read_text_safe(path: str) -> str:
    """多编码安全读取（R3+R5 合规）"""
    return _read_text_safe_cached(path)


def _iter_lines(path: str, encoding: Optional[str] = None):
    """流式读取文件行，支持编码探测"""
    if encoding is None:
        # 复用 _read_text_safe 的编码探测逻辑
        for enc in ("utf-8", "gbk", "gb18030"):
            try:
                with open(path, encoding=enc, errors="replace") as f:
                    for line in f:
                ...
                continue
        raise SchemrError("E002", f"文件读取失败: {path}")
    else:
        try:
            ...
            raise SchemrError("E002", f"文件读取失败: {path} - {exc}")
...
def parse_input_source(source: str) -> Any:
    ...
    if source.startswith(("http://", "https://")):
        content = fetch_url_with_retry(source)
        # 根据内容尝试解析
        try:
            return parse_json_text(content)
        except SchemrError:
        ...
        try:
            ...
            pass
        raise SchemrError("E003", f"URL内容无法解析: {source}")
