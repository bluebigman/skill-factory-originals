...
# -*- coding: utf-8 -*-
"""
meeting-minutes-gen — 配套执行器（原创实现，clean-room）
技能「meeting-minutes-gen」的轻量辅助脚本：解析同目录 SKILL.md，提供 CLI 入口、触发词匹配、能力速览。
零第三方依赖。
"""
...
def selftest() -> int:
    ...
    assert got, "触发匹配失败"
    print("  [OK] 触发匹配:", got)
    print("== meeting-minutes-gen 配套执行器自检通过 ✅ ==")
    return 0


def main():
    ap = argparse.ArgumentParser(description="meeting-minutes-gen 配套执行器")
