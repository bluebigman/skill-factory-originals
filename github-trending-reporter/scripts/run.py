...
# -*- coding: utf-8 -*-
"""
github_trending_reporter — 配套执行器（原创实现，clean-room）
技能「github-trending-reporter」的轻量辅助脚本：解析同目录 SKILL.md，提供 CLI 入口、触发词匹配、能力速览。
零第三方依赖。
"""
...
TRIGGERS = ["github-trending-reporter", "github趋势", "trending榜单", "本周热门项目", "开源项目周报", "最近有啥火的开源项目", ""]
...
def selftest() -> int:
    ...
    assert got, "触发匹配失败"
    print("  [OK] 触发匹配:", got)
    print("== github_trending_reporter 配套执行器自检通过 ✅ ==")
    return 0


def main():
    ap = argparse.ArgumentParser(description="github_trending_reporter 配套执行器")
