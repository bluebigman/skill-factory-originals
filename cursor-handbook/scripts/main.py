    ...
    def __post_init__(self):
        ...
        if not self.created_at:
            ...
            try:
                ...
                self.created_at = dt.astimezone(timezone.utc).isoformat()
            except ValueError:
                # 解析失败，抛出异常并携带原始字符串上下文
                raise ValueError(
                    f"时间戳格式无效: '{self.created_at}' (规则 {self.rule_id})，"
                    f"期望 ISO 8601 格式，例如 2024-01-01T00:00:00+00:00"
                )

    def to_dict(self) -> Dict[str, Any]:
        """转换为字典（用于JSON/YAML输出）"""
        # 直接使用已规范化的 created_at，避免重复解析
        return asdict(self)
...
def convert_to_skill(rules: List[RuleEntry], template: Optional[str] = None) -> str:
    ...
    skill_doc += f"> 生成时间: {datetime.now(timezone.utc).isoformat()}\n\n"
    skill_doc += "## 规则总览\n\n"
...
def output_rules(rules: List[RuleEntry], output_format: str = "json") -> str:
    ...
    if output_format == "json":
        ...
        if not HAS_YAML:
            raise RuntimeError(
                "E008: YAML 输出需要 PyYAML 库，请先安装: pip install pyyaml"
            )
        # 使用 PyYAML 进行序列化
        return yaml.dump(
        ...
        md_lines = []
        for rule in rules:
            md_lines.append(f"## {rule.rule_id}: {rule.name}")
            md_lines.append("")
            md
