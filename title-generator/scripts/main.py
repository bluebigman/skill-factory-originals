# -*- coding: utf-8 -*-
"""main.py — 爆款标题生成器 CLI 引擎（规则模板生成器）
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(
        prog="title-generator",
        description="按内容主题生成数字、悬念、对比、清单等类型爆款标题。（离线模板引擎，不写盘预览优先）",
    )
    p.add_argument("--topic", default="", help="主题/内容（必填一：topic 或 input）")
    p.add_argument("--input", default="", help="输入文件路径（多编码容错；与 topic 二选一）")
    p.add_argument("--platform", default="generic", choices=sorted(SCENE_OPTIONS.keys()))
    ...
    p.add_argument("--version", action="version", version="title-generator " + VERSION)
...
def _safe_main(argv=None):
    ...
    for idx in range(args.count):
        docs.append(build_doc(raw, args.platform, idx, rng, log))

    scene_label = SCENE_OPTIONS.get(args.platform, args.platform)
    if args.as_json:
        payload = json.dumps({"generator": "title-generator", "version": VERSION,
