...
def process_batch(
    items: List[Tuple[str, str, Optional[Dict[str, str]], Optional[List[str]], bool]],
    max_workers: int = 4,
    timeout: float = 30.0,
) -> List[str]:
    """并发处理多条数据。items 为 (input_text, output_format, field_mapping, date_fields, remove_duplicates) 元组列表。"""
    results: List[Tuple[int, str]] = []
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_index = {
            executor.submit(process_data, text, fmt, mapping, dates, dedup): idx
            for idx, (text, fmt, mapping, dates, dedup) in enumerate(items)
        }
